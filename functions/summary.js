// SLL Services - Expense Summary Worker
// Handles both weekly and monthly summary emails
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const records = await fetchAirtableRecords(env.AIRTABLE_API_KEY, startDate, endDate);
      ...
      const html = buildEmailHTML(summary, periodLabel, type, startDate, endDate);
      
      await sendEmail(env, html, type, periodLabel);
  ...
  async scheduled(event, env, ctx) {
    ...
    // Monday = weekly report (covers previous Sun-Sat)
    if (dayOfWeek === 1) {
      await runSummary(env, 'weekly');
    }

    // 1st of month = monthly report
    if (dayOfMonth === 1) {
      await runSummary(env, 'monthly');
    }
  }
};

async function runSummary(env, type) {
  const { startDate, endDate, periodLabel } = getDateRange(type);
  const records = await fetchAirtableRecords(env.AIRTABLE_API_KEY, startDate, endDate);
...
async function fetchAirtableRecords(apiKey, startDate, endDate) {
  const formula = encodeURIComponent(
    `AND(IS_AFTER({Submitted At}, '${startDate}T00:00:00.000Z'), IS_BEFORE({Submitted At}, '${endDate}T23:59:59.000Z'))`
  );
...
function buildSummary(records) {
  ...
  records.forEach(record => {
    ...
    if (amount >= FLAG_THRESHOLD) {
      flagged.push({ employee, vendor, amount, category, purpose, jobCode, date });
    }
  });

  // Sort categories and employees by amount descending
...
function buildEmailHTML(summary, periodLabel, type, startDate, endDate) {
  ...
  const flaggedSection = summary.flagged.length > 0 ? `
    <div style="margin-top:30px;">
      <h3 style="font-family:Arial,sans-serif;color:#c0231e;border-bottom:2px solid #c0231e;padding-bottom:8px;">
        ⚠️ Expenses Over $${FLAG_THRESHOLD.toLocaleString()}
      </h3>
...
async function sendEmail(env, html, type, periodLabel) {
  const reportType = type === 'weekly' ? 'Weekly' : 'Monthly';
  
  const emailPayload = {
    service_id: 'service_e1lex4v',
    template_id: 'template_plzb7ak',
    user_id: 'fBr3U4xpS_U3gH4og',
    template_params: {
